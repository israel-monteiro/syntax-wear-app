...
export const CEPForm = () => {
    ...
    return (
        <>
            <form className="flex gap-3 items-start" onSubmit={handleSubmit(onSubmit)} noValidate>
                <div className="flex flex-col flex-1">
                    <input
                        type="text"
                        placeholder="Insira seu CEP"
                        className={`border border-[#c0c0c0] rounded-md p-3 ${errors.cep ? "border-red-500" : ""}`}
                        {...register("cep")}
                    />

                    {errors.cep && <span className="text-red-600 text-sm mt-1 ">{String(errors.cep.message)}</span>}
                </div>
